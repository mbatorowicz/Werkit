...
- **Komunikaty UI**: Potwierdzenia i alerty przez `useAppDialog()` (`AppDialogProvider` w root layout) — nie używaj `window.alert` / `window.confirm`. Formularze w modalach: `AdminModalShell` + `FormModalFooter` (szczegóły: `AGENTS.md` §6).
