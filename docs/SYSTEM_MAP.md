...
- **PWA**: `public/manifest.json` + ikony PNG 192/512; favicon karty: `src/app/icon.tsx` (`/icon`, emerald W na zinc-900) + fallback `public/favicon.ico` (metadata `icons.icon` w `layout.tsx` — bez tego Next nadpisuje `rel="icon"` samym apple-touch); SW `werkit-v5` — install nie pada na 404, GET sesji/zleceń/ustawień = **network-first**. Rejestracja SW tylko w przeglądarce (nie w Capacitor WebView).
...
| `npm run icons:generate` | Ikony PWA, favicon ICO/PNG 32 oraz assety Google Play | `generate_pwa_icons.ts` |
