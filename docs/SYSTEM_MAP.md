...
- **PWA**: `public/manifest.json` + ikony PNG 192/512; favicon: `public/favicon.ico` + `public/icons/favicon-32.png` (metadata `icons.icon` w `layout.tsx` — bez tego Next nadpisuje `rel="icon"` samym apple-touch); SW `werkit-v4` — install nie pada na 404, GET sesji/zleceń/ustawień = **network-first**. Rejestracja SW tylko w przeglądarce (nie w Capacitor WebView).
