...
export function parseAndroidApkMeta(raw: unknown): AndroidApkMeta | null {
  ...
  const packageVersion =
    typeof o.packageVersion === 'string' ? o.packageVersion.trim() : version;
  const buildType = o.buildType === 'release' ? 'release' : o.buildType === 'debug' ? 'debug' : null;
  if (!version || !buildType) return null;
  const commitSha = typeof o.commitSha === 'string' && o.commitSha.trim() ? o.commitSha.trim() : undefined;
