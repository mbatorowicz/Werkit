...
import { Trash2, Shield, Plus, X, Lock, Edit2 } from "lucide-react";
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <button onClick={openNewModal} className="bg-white text-zinc-950 px-5 py-2.5 text-sm font-semibold rounded-xl hover:bg-zinc-200 transition shadow-sm flex items-center gap-2">
          ...
          <table className="w-full text-left border-collapse min-w-[600px]">
             <thead>
               <tr className="border-b border-zinc-800/50 bg-[#0a0a0b]/80">
                 ...
                 <th className="px-6 py-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Login Systemowy</th>
                 <th className="px-6 py-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider text-right">Zarządzanie</th>
               </tr>
             </thead>
             <tbody className="divide-y divide-zinc-800/50">
               {isLoading ? (
                 <tr><td colSpan={3} className="px-6 py-12 text-center text-zinc-500 text-sm">Pobieranie bazy kadr...</td></tr>
               ) : workers.map(worker => (
                 <tr key={worker.id} className="hover:bg-zinc-800/20 transition-colors">
                   <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                           ...
                           <div className="text-xs text-zinc-500 mt-0.5">{worker.role === 'admin' ? 'Administrator' : 'Fizyczny Pracownik'}</div>
                         </div>
                      </div>
                   </td>
                   <td className="px-6 py-4">
                   ...
                   <td className="px-6 py-4 text-right">
                     <div className="flex justify-end gap-1">
                        <button onClick={() => handeEditClick(worker)} className="p-2 text-zinc-500 hover:text-amber-500 hover:bg-amber-500/10 rounded-lg transition" title="Edytuj dane i hasło">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        {worker.role !== 'admin' && (
                          <button onClick={() => handleDelete(worker.id, worker.fullName)} className="p-2 text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition" title="Usuń konto na stałe">
                 ...
                 <div className="grid grid-cols-2 gap-4 pt-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input required={!editId} type="text" placeholder={editId ? "*** (ukryte)" : "1234"} value={form.password} onChange={e => setForm({...form, password: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2.5 pl-10 text-white focus:ring-1 focus:ring-amber-500 focus:border-amber-500 transition outline-none" />
                      </div>
                    </div>
                 </div>

                 {editId ? (
                    <div className="bg-amber-500/10 p-4 border border-amber-500/20 rounded-xl mt-4 text-xs text-amber-500 leading-relaxed font-medium">
