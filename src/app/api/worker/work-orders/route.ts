...
export const GET = withApiErrorHandling(async (request: Request) => {
  ...
  const orders = await WorkerOrderService.getPendingOrders(ctx.userId, ctx.companyId, { offset, limit });
...
export const POST = withApiErrorHandling(async (request: Request) => {
  ...
  mapUnknownError: (err) => {
    ...
    if (err instanceof Error && err.message === "missing_fields") return jsonError("missing_fields", 400);
    if (err instanceof Error && err.message === "invalid_payload") return jsonError("invalid_payload", 400);
