...
export const PUT = withApiErrorHandling(
  async (req: Request, ctx: { params: Promise<{ id: string; locationId: string }> }) => {
    ...
    if (body.address !== undefined)
      patch.address = typeof body.address === "string" ? body.address : null;
    if (body.latitude !== null && body.latitude !== undefined)
      patch.latitude = String(body.latitude);
...
export const DELETE = withApiErrorHandling(
  async (_req: Request, ctx: { params: Promise<{ id: string; locationId: string }> }) => {
    const denied = await guardAdminMutation();
    if (denied) return denied;
