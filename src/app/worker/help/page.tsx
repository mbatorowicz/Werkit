    ...
    <div className="py-6 pb-20">
      ...
      <div className="space-y-4 mb-8">
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-zinc-600 dark:text-zinc-300 text-sm mb-4">{dict.contactDesc}</p>
          ...
          <a
            href={`tel:${phone}`}
            className="w-full bg-blue-50 dark:bg-blue-500/10 hover:bg-blue-100 dark:hover:bg-blue-500/20 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/30 rounded-lg p-4 flex items-center justify-center gap-3 transition-colors font-medium"
          >
      ...
      <div className="space-y-3">
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1 mt-2">
            <li>
              <strong>{dict.redColor}</strong> {dict.redColorDesc}
            </li>
            <li>
              <strong>{dict.pinkColor}</strong> {dict.pinkColorDesc}
            </li>
            <li>
              <strong>{dict.priorities}</strong> {dict.prioritiesDesc}
            </li>
          </ul>
          <p className="mt-3">
            {dict.startWorkInstruction}
            <strong>ROZPOCZNIJ ZADANIE</strong>
            {dict.startWorkInstruction2}
          </p>
        </HelpAccordion>

        <HelpAccordion
          title={dict.notesAndPhotos}
          icon={<Camera className="w-5 h-5 text-blue-600 dark:text-blue-400" />}
        >
          <p className="mb-2">{dict.notesAndPhotosDesc}</p>
          ...
          <p className="mt-3 text-amber-600 dark:text-amber-500 font-medium">{dict.photoWarning}</p>
        </HelpAccordion>

        <HelpAccordion
          title={dict.gpsTracking}
          icon={<Navigation className="w-5 h-5 text-blue-600 dark:text-blue-400" />}
        >
          <p className="mb-2">{dict.gpsTrackingDesc}</p>
          <ul className="list-disc pl-5 space-y-1 mt-2">
            <li>
              <strong>{dict.gpsWait}</strong> {dict.gpsWaitDesc}
            </li>
            <li>
              <strong>{dict.gpsActive}</strong> {dict.gpsActiveDesc}
            </li>
          </ul>
          <p className="mt-3">
            {dict.gpsPrivacy}
            <strong>{dict.gpsPrivacy2}</strong>
            {dict.gpsPrivacy3}
          </p>
        </HelpAccordion>

        <HelpAccordion
          title={dict.customOrders}
          icon={<Info className="w-5 h-5 text-blue-600 dark:text-blue-400" />}
        >
          <p>
            {dict.customOrdersDesc1}
            <strong>{dict.customOrdersDesc2}</strong>.
          </p>
          <p className="mt-2">{dict.customOrdersDesc3}</p>
        </HelpAccordion>
      </div>
      ...
      <div className="mt-8 bg-orange-50 dark:bg-orange-500/10 border border-orange-200 dark:border-orange-500/30 rounded-2xl p-5 shadow-sm">
        ...
        <p className="text-xs text-orange-800 dark:text-orange-300">{dict.emergencyDesc}</p>
      </div>
    </div>
  );
}
