...
describe("PlatformCompanyService.mapCreateError", () => {
  it("mapuje unikalny slug", () => {
    const err = Object.assign(new Error('duplicate key value violates unique constraint "companies_slug_key"'), {
      code: "23505",
    });
  ...
  it("mapuje unikalny email admina", () => {
    const err = Object.assign(new Error('duplicate key value violates unique constraint "users_username_email_key"'), {
      code: "23505",
    });
