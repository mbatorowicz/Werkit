...
interface StockIssuesTableProps {
  issues: (StockIssue & { partName?: string; partCatalogNumber?: string; creatorName?: string; workOrderLabel?: string })[];
  dict: {
    table: {
      date: string;
      part: string;
      ...
      <table className="w-full text-sm">
        <thead className="bg-zinc-50 dark:bg-zinc-800/50">
          <tr>
            ...
            <th className="px-4 py-3 text-left font-medium text-zinc-600 dark:text-zinc-400">{dict.table.workOrder}</th>
            <th className="px-4 py-3 text-left font-medium text-zinc-600 dark:text-zinc-400">{dict.table.issuedTo}</th>
