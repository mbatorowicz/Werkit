...
export type WorkerSessionActionDeps = {
...
export async function endSessionAction(
  ...
  if (args.settings?.requirePhotoToFinish) {
    const hasPhoto = args.timelineEvents.some((e) => e.type === "photo");
    if (!hasPhoto) {
  ...
  setIsLoading(true);
  try {
    const body = args.endLocation
      ? { latitude: args.endLocation.lat, longitude: args.endLocation.lng }
      : {};
...
export async function acceptOrderAction(
  ...
  args: {
    ...
    setAcceptErrors: React.Dispatch<React.SetStateAction<Record<number, string>>>;
  }
): Promise<void> {
  const { dict, apiErrors, appAlert, fetchSessionAndPath, setIsLoading } = deps;
  const { orderId, startLocation, setAcceptErrors } = args;
  setIsLoading(true);
...
export async function cancelSessionAction(deps: WorkerSessionActionDeps): Promise<void> {
  const { dict, appAlert, appConfirm, fetchSessionAndPath, setIsLoading } = deps;
...
export async function saveNoteAction(
  ...
  args: {
    ...
    setIsSubmittingNote: (val: boolean) => void;
    setIsNotesModalOpen: (val: boolean) => void;
    setNoteText: (val: string) => void;
    setEditingNoteId: (val: number | null) => void;
  }
): Promise<void> {
  const { dict, appAlert, fetchSessionAndPath } = deps;
  const { location, noteText, editingNoteId } = args;
  if (!noteText.trim()) return;
  args.setIsSubmittingNote(true);
  try {
    ...
    if (result.queued) {
      ...
      args.setIsNotesModalOpen(false);
      args.setNoteText("");
      args.setEditingNoteId(null);
    ...
    args.setIsSubmittingNote(false);
  }
}

export async function photoUploadAction(
  deps: WorkerSessionActionDeps,
  args: { e: React.ChangeEvent<HTMLInputElement>; location: Coord | null }
): Promise<void> {
  const { dict, appAlert, fetchSessionAndPath, setIsLoading } = deps;
  const { e, location } = args;
...
async function compressPhotoToBase64(file: File): Promise<string> {
