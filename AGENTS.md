...
2. **Uprawnienia i Middleware**: 
   - Cały system opiera się o tokeny i `src/middleware.ts`. 
   - Nie nadpisuj ani nie wyłączaj middleware. Jeśli dodajesz publiczne / współdzielone endpointy (dla maszyn, klientów itp.), dodawaj je wyraźnie do reguły `isSharedApi` w middleware, aby pracownicy z aplikacją mobilną nie otrzymywali błędów HTTP 403.
